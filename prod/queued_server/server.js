...
let server = app.listen(4600, function () {
...
let handle_get = async function(req, res) {
        let auth = req.header('AuthToken');
	let sesh = req.header('SessionID');
        let path = `logs/default.txt`;
	...
	if (sesh == 'confirmuser') {
	    await validate_user(req, res, path);
	}
        else if (sesh == 'getuserid') {
	    await get_user_id(req, res, path);
	}
	else if (sesh == 'getauth') {
	     await get_auth(req, res, path);
	}
	else if(sesh == 'getcode') {
    	    await get_code(req, res, path);
	}
        else if(sesh == 'checkconnection') {
	    await res.send({active: true});
	}
	else {
            verify_auth(auth, path, async (result) => {
		let verify = result;
		...
		if (verify == false ||  verify == 0 ||
			...
			case 'confirmuser':
			    ...
			    break;
			case 'getcontacts':
			    await  get_contacts(req, res, path);
			    break;
			case 'geteventcontacts':
			    await get_event_contacts(req, res, path);
			    break;
			case 'getfavoriteevents':
			    await get_favorite_events(req, res, path);
			    break;
			case 'getinactiveevents':
			    await get_inactive_events(req, res, path);
			    break;
			case 'getlastinactiveevent':
			    await get_last_inactive_event(req, res);
			    break;
			case 'getactiveevent':
			    await get_active_event(req, res, path);
			    break;
			case 'getinactiveeventendloc':
			    await get_inactive_event_end_loc(req, res, path);
			    break;
			case 'getauth':
			    await get_auth(req, res, path);
			    break;
			case 'getemail':
			    await get_email(req, res, path);
			    break;
			case 'getuser':			    
			    await get_user(req, res, path);
			    break;
			case 'getuserid':
			    await get_user_id(req, res, path);
			    break;
			case 'getaccount':
			    await get_account_info(req, res, path);
			    break;
			case 'deleteevent':
                            delete_event(req, path);
                        break;
...
let handle_post = async function(req, res) {
    let auth = req.header('AuthToken');
    let sesh = req.header('SessionID');
    let body = req.body;
    let path = `logs/default.txt`;
    ...
    else if (sesh == 'forgotpassword') {
	forgot_password(body, res, path);
    } 
    else if (sesh == 'getusername') {
	get_username(body,res,path);
    }
    else if(sesh == 'getcode') {
    	get_code(body, res, path);
    }
    else if(sesh == 'resetpassword') {
	reset_password(body, path);
		...
		if (auth == null) {
		    ...
		    switch(sesh) {
		    case 'deletecontact':
			await delete_contact(body, path);
			break;
		    case 'newcontact':
			 await create_contact(body, path);
			break;
		    case 'newevent':
			await create_event(body, path);
			//res.end();
			break;
		    case 'disableevent':
			await disable_event(body, path);
			break;
		    case 'favoriteevent':
			await favorite_event(body, path);
			break;
		    case 'updatecontact':
			await update_contact(body, path);
			break;
		    case 'createuser':
			await create_user(body, res, path);
			break;
		    case 'resetpassword':
			await reset_password(body, path);
			break;
		    case 'createcontactinfo':
			await create_contact_info(body, path);
			break;
		    case 'updateaccount':
			await update_account(body, path);
			break;
		    case 'forgotpassword':
			await forgot_password(body, res, path);
			break;
		    case 'getusername':
			await get_username(body, res, path);
			break;
		    case 'deleteevent':
                        delete_event(req, path);
                        break;
		    case 'getcode':
			await get_code(body, res, path);
			break;
	            case 'disableuser':
                        disable_user(body, path);
                        break;
		    default:
			res.end('invalid sessionid');
			break		
...
async function verify_auth(auth, path, callback) {
...
async function get_auth(req, res, path) {
...
async function get_auth_w_user(username, res, path) {
    sql.close();
    sql.connect(config, function () {
        let request = new sql.Request();
        request.query(` SELECT Auth FROM BCUser
...
async function generate_auth(username, path) {
 ...
 function  disable_user(body, path){
        sql.close();
        var userid = body.userid;
        sql.connect(config,  function (err) {
        if (err) throw err;
        var request = new sql.Request();
            Log("Delete User Query", path);
        request.query(`
        DECLARE @event_transact varchar(20) = 'DeleteUser';
        ...
        EXEC sp_executesql @delete_one_sql, @params, @userid = ${userid}

        COMMIT TRAN @event_transact;
        `).then( function  () {
            Log("Deleted user", path);
            sql.close();
        }).catch( function  (err) {
            Log(err, path);
            sql.close();
...
async function update_contact(body, path) {
    sql.close();
...
async function delete_contact(body, path){
        sql.close();
        let id = body.id;
        sql.connect(config, function(err) {
        if (err) throw err;
...
async function create_event(body, path){
	sql.close();
	let userid = body.userid;
        Log('Creating event for UserID: ' + userid, path);
	let event_name = body.name;
	let event_lat = body.startLat;
	let event_long = body.startLong;	
	let event_desc = body.description;
	let event_start = body.startDate;
        let event_end = body.endDate;
	let event_end_lat = body.endLat;
	let event_end_long = body.endLong;
	let contacts_list = body.contactsList;
...
BEGIN TRAN @event_transact;
    ...
    SET @sql1 = N'INSERT INTO Position (PositionLatitude, PositionLongitude) VALUES (@end_lat, @end_lng);';
    ...
    @start_date = N'${event_start}',
    @event_name = N'${event_name}',
    @participants = N'${participants}',
    @event_desc = N'${event_desc}';
...
BEGIN
        ...
        SET @InsertEventContactInfo = 'INSERT INTO Event_Contact (EmergencyContactID, EventID) VALUES (' + @id + ', \
' + CAST(@event_id as nvarchar) + ');'
...
COMMIT TRAN @event_transact;
`).then(function () { // (' + @id + ', ' + CAST(@event_id as nvarchar) + ');'
                Log("Event Created", path);
		sql.close();
	}).catch(function (err) {
		Log(err, path);
		sql.close();
...
async function get_contacts(req, res, path){
    sql.close();
	sql.connect(config, function () {
        let request = new sql.Request();
        request.query(`
        ...
        @id = ${req.header('userID')}
		 ...
		 {
           	     if (err) Log(err, path);
              	     res.send(recordset); // Result in JSON formay
	    	 });	
	});
}

async function get_favorite_events(body, res, path) {
    sql.close();
    sql.connect(config, function () {
        let request = new sql.Request();
        request.query(`
 DECLARE @sql nvarchar(4000),
       ...
       SET @sql =
        'SELECT BCevent.EventID, EventName, EventStartDate, EndDate, a.PositionLatitu\
de AS StartLat,
        a.PositionLongitude AS StartLon, b.PositionLatitude AS EndLat,
        b.PositionLongitude AS EndLon, EventDescription, EventParticipants, IsCurrent, IsFavorite       \
        ...
        WHERE BCEvent.UserID = @id AND BCEvent.IsFavorite = 1;'

        EXEC sp_executesql @sql, @params,
        @id = ${body.header('userID')}
...
async function get_event_contacts(req, res, path){
    sql.close();
    sql.connect(config, function () {
	let request = new sql.Request();
	request.query(`
...
async function get_inactive_events(req, res, path) {
	sql.close();
        sql.connect(config, function () {
        let request = new sql.Request();
        request.query(`
       ...
       SET @sql =         
        'SELECT BCevent.EventID, EventName, EventStartDate, EndDate, a.PositionLatitude AS StartLat,
        a.PositionLongitude AS StartLon, b.PositionLatitude AS EndLat,
        b.PositionLongitude AS EndLon, EventDescription, EventParticipants, IsCurrent, IsFavorite        
        FROM BCEvent
        JOIN Position a
        ...
        JOIN Position b
        ON b.PositionID = EndInfo.EndPositionID        
        WHERE BCEvent.UserID = @id AND BCEvent.IsCurrent = 0 AND BCEvent.IsDisabled = 0;'
...
async function get_last_inactive_event(req, res, path) {
        sql.close();
        sql.connect(config, function () {
        let request = new sql.Request();
        request.query(`
...
async function get_active_event(req, res, path) {
    sql.close();
    sql.connect(config, function () {
        let request = new sql.Request();
        request.query(`
        ...
        EventParticipants, EventDescription, IsFavorite
        FROM BCEvent
            JOIN Position a
        ON a.PositionID = BCEvent.CreationPositionID
            JOIN EndInfo
...
async function get_active_event_loc(body, path) {
        sql.close();
	sql.connect(config, function () {
		let request = new sql.Request();
		request.query(`
...
async function disable_event(body, path) {  
...
//Favorite an event
async function favorite_event(body, path) {
	let eventid = body.eventID;
	sql.close();
	sql.connect(config, function(err)
	{
...
//creates user
async function create_user(body, res, path) {
    sql.close();
...
*/
//change password
async function reset_password(body, path) {
       sql.close();
       
	let username = body.username.replace(/\"/g, "");
        console.log("password :", body.nPassword); 
	let password = body.nPassword;
...
async function get_user(req, res, path) {
    let user = req.header('un-content');
    sql.close();
    sql.connect(config, function() {
	let request = new sql.Request();
	request.query(`
...
async function get_email(req, res, path) {
         Log("In get GetEmail", path);
    let email = req.header('un-content');
        sql.close();
        sql.connect(config, function() {
        let request = new sql.Request();
        request.query(`
                ...
                SET @sql = 'SELECT UserID from BCUser
                            WHERE Email = @email';
...
//gets user ID
async function get_user_id(req, res, path) {
    Log("In get UserID", path);
    let username = req.header('un-content');
   	sql.close();
	sql.connect(config, function() {
	let request = new sql.Request();
	request.query(`
...
async function validate_user(req, res, path) {
    let password = req.header('pw-content');
    let username = req.header('un-content');
	...
	sql.connect(config, function() {
        ...
        function (err, result){
	sql.close();
            let confirmed = bcrypt.compareSync(password, result.recordset[0].hashPass);
...
async function create_contact_info(body, path) {
        sql.close();
        let first_name = body.firstName;
        let last_name = body.lastName;
        let phone_number = body.phoneNumber;
...
//Get account info
async function get_account_info(req, res, path) {
...
async function update_account(body, path) {
        sql.close();
        let id = body.userID;
        let fname = body.firstName;
        let lname = body.lastName;
...
async function forgot_password(body, res, path){
	sql.close();
	let email = body.email;
        let username = body.username._body.toString().replace(/\"/g, "");
	console.log("here1", username);
	console.log("here2",email);

	let key = Math.random().toString(36).replace('0.', '');
	 
	randomString = generate_random_string(9);
	sql.connect(config, function(err){
	if(err) throw err;
	let request = new sql.Request();
	request.query(`
	DECLARE @user_transact varchar(20) = 'forgotpassword',
		@sql nvarchar(4000),
		@params nvarchar(4000);
	BEGIN TRAN @user_transact


	SET @params = '@email nvarchar(255),
			@randomString nvarchar(255),
			@username nvarchar(255)';
			
	SET @sql = 'INSERT INTO dbo.forgotpassword (email, string, UserName)
			VALUES(@email, @randomString, @username)';
...
async function get_username(body, res, path) {
	...
	let username = data.recordset[0].UserName;
...
async function get_code(body, res, path) {
	sql.close();
	let code = body.userCode;
	let Promise1 = new Promise(function(resolve, reject) {
	sql.connect(config, function() {
	let request = new sql.Request();
	request.query(`
...
async function generate_random_string(string_length) {
	let random_string = '';
	let random_ascii;
	let ascii_low = 65;
	let ascii_high = 90;
    ...
    return random_string
     ...
     function  delete_event(req, path) {
         var body = req.body;
	 var eventid = body.eventid;
        Log('Deleting event: ' + eventid, path);
	if (eventid > 0) { 
        sql.close();
        sql.connect(config,  function (err)
        {
