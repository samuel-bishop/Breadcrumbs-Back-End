...
let sql = new ConnectionPool(sql_config);
...
server.get('/getData/', async function (req, res) {
    setTimeout(function() {
                ...
                if (verify == false ||  verify == 0 ||
                    ...
                    if (auth == null) {
                        ...
                        case 'getfavoriteevents':
                            get_favorite_events(req, res, path);
                            break;
...
server.post('/updateData/', async function(req, res) {
    setTimeout(function (){
        var auth = req.header('AuthToken');
        var sesh = req.header('SessionID');
        var body = req.body;
...
function get_favorite_events(body, res, path) {
    sql.close();
    sql.connect(config, function () {
        var request = new sql.Request();
        request.query(`
...
function reset_password(body, path) {
       sql.close();
        ...
        var username = body.username.replace(/\"/g, "");
...
function forgot_password(body, res, path){
        sql.close();
        var email = body.email;
        var username = body.username._body.toString().replace(/\"/g, "");
